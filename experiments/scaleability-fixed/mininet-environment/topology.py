...
class GenericMiddleBoxTopology(object):
    ...
    def __init__(self, source_instances=1, target_instances=1, mbox_instances=2):
    ...
    def run_target_hosts(self):
        ...
        Attention: Supports only one host!
        Starts one process for each MB!
        """
        th = self.target_hosts[0]
        sh = self.source_hosts[0]
        p = USER_BASE_PORT
        for mb in self.middlebox_hosts:
