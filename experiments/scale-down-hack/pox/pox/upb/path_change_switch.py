...
Scenario:
 * t = 0: flow c1 through mb1; flow c2 through m2
 * t = 60: move! flows of c1, c2 pass through mb1
...
def countdown_thread(sc):
    ...
    for i in range(FLOW_MOVE_DELAY, 0, -10):
        ...
        time.sleep(10)
    # move client 2 back to mb1
    sc.delete_flow_move_1()
...
class SwitchController(object):
    ...
    def delete_flow_move_1(self):
        log.info("Flow move c2 on s%d" % self.switch)
        m = self.mapping
        if self.switch == 2:
            # move flow from client2 to second link
            self.del_static_rule(
                m["s2-eth4"], m["s2-eth2"], 10, dl_type=0x800, nw_src="20.0.0.2", nw_dst="20.0.1.1")
        elif self.switch == 3:
            # move flow from client2 to second link
            self.del_static_rule(
